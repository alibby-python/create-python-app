...
def prompt(
    ...
    prompt_text = f"{prompt_colour}❯{RESET} {text_colour}{message}{RESET}"
...
def filepath_prompt(
    ...
    while True:
        ...
        print(apply_markup(f"{{red}}❌ {error_message}{{/red}}"))
        print()
...
def select_prompt(
    ...
    print(apply_markup(f"{{cyan}}❯{{/cyan}} {message}:"))
    ...
    while True:
        ...
        if keypress.key == Key.UP:
            ...
            print(
                apply_markup(
                    f"{{cyan}}❯{{/cyan}} {message}: {{green}}{selected}{{/green}}"
                )
            )
...
def checkbox_prompt(
    ...
    instruction: str | None = ("SPACE = Toggle / ENTER = Confirm"),
) -> list[str]:
    """
    ...
    if instruction:
        print(apply_markup(f"   {{blue}}💡 {instruction}{{/blue}}"))

    def render() -> None:

        for index, option in enumerate(options):
            checked = "☑" if index in selected_items else "☐"

            prefix = "❯" if index == selected_index else " "

            line = f"  {prefix} {checked} {option}"

            if index == selected_index:
                print(apply_markup(f"{{cyan}}{line}{{/cyan}}"))
            else:
                print(line)
    ...
    while True:
        ...
        if keypress.key == Key.UP:
            ...
            selected = [options[index] for index in sorted(selected_items)]
            ...
            print(
                apply_markup(
                    f"{{cyan}}❯{{/cyan}} {message}: {{white}}{summary}{{/white}}"
                )
            )
